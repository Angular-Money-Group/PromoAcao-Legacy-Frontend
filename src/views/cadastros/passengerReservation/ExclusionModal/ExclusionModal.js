...
const ExclusionModal = ({
  ...
  const exclusionModalStore = useSelector(state => state.passengerInReservationExclusionModal)
  const modalIsOpen = exclusionModalStore.modalIsOpen
  const passengerId = exclusionModalStore.passengerId
  ...
  const onSubmit = () => {
    if (passengerId !== null) {
      axios.delete(`/api/reservations/passengers/${passengerId}`)
        .then(() => {
          setShow(!show)
          reset()
          toast(
            <div className='d-flex align-items-center'>
              ...
              <div className='d-flex flex-column'>
                <h6 className='mb-0'>Passageiro excluído com sucesso!</h6>
              </div>
            </div>
          )
          dispatch(
